...
from racecar_gym_main.racecar_gym import SingleAgentScenario
from racecar_gym_main.racecar_gym.envs.single_agent_race import SingleAgentRaceEnv
from utils.config import Config
import os
from FTG import GapFollower
...
def learning_curve_display(epoch, last_show_num, logger, train_pro_list):
    ...
    if epoch / last_show_num > display_frequency:
        plt.cla()
        plt.title(train_mode, loc='center')
        plt.xlabel('Epoch', loc='center')
        plt.ylabel('Lap Progress', loc='center')
        plt.plot(train_pro_list, label="train_progress")
        plt.savefig('logs/'+train_mode+'/figure.png')
        plt.legend()
        last_show_num = epoch
...
def ppo(env_fn, actor_critic=core.mlp_actor_critic, ac_kwargs=dict(), seed=0,
        steps_per_epoch=10000, epochs=50, gamma=0.99, clip_ratio=0.2, pi_lr=3e-4,
    ...
    adv_ph, ret_ph, logp_old_ph = core.placeholders(None, None, None)    #log_old_ph
    ...
    if new_train_flag:
        ...
        start_steps = 0
        network = tf.compat.v1.trainable_variables()
    ...
    for epoch in range(epochs):
        ...
        for t in range(local_steps_per_epoch):
            ...
            # ----------------------------------------------------------------------------------------------------------#
            a_residual = a
            a_prior = G.__call__(lidar_obs)
            a_c = a_prior*(1+(beta*a_residual))
            a = np.clip(a_c, -1, +1)
            ...
            if delta_progress != 0:
                progress_history = progress_history + [delta_progress]

            if len(progress_history)>100:

                if count(progress_history) > 90:
                    d = True
                    progress_history = []
...
if __name__ == '__main__':
    ...
    parser.add_argument('--exp_name', type=str, default=alg)
